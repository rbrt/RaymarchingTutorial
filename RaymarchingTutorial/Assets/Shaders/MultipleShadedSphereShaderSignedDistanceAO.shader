Shader "Custom/MultipleShadedSphereShaderSignedDistanceAO"
{
	Properties
	{
		...
		_AOStepSize("AOStepSize", Float) = .001
		_AOStepCount("AOStepCount", Float) = 10
	}
	SubShader
	{
		...
		Pass
		{
			...
			float map(float3 p)
			{
				return sdfBlend(sdf1(p), sdf1(p), _SinTime.w * _SinTime.w);
			}
			...
			float ambientOcclusion (float3 pos, float3 normal)
			{
			    float sum    = 0;
			    float maxSum = 0;
			    for (int i = 0; i < (int)_AOStepCount; i++)
			    {
